...
if element == 'Wage':
    ptlist = df['GEO'].drop_duplicates().sort_values().to_list()
    ...
    year = st.sidebar.multiselect(label='Select Year', options=bc['Year'].drop_duplicates().sort_values(ascending=False),
                                  default=[2021])
    ptlist = bc['variable'].drop_duplicates().sort_values().to_list()
    ptlist.remove('Canada')
    ptlist.insert(0, 'Canada')
    pt = st.sidebar.selectbox(label='Select Region', options=ptlist)
    ...
    year = st.sidebar.multiselect(label='Select Year',
                                  ...
                                  default=[2021])
    ptlist = em['GEO'].drop_duplicates().sort_values().to_list()
    ptlist.remove('Canada')
    ptlist.insert(0, 'Canada')
    pt = st.sidebar.selectbox(label='Select Region', options=ptlist)
    ...
    data = data[data['NAICS'] != 'Services-producing sector']
    data = data[data['NAICS'] != 'Goods-producing sector']
    data = data.sort_values('VALUE')
